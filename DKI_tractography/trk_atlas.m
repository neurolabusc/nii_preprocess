function [m] = trk_add_sc_Emilie(header,tracks,volume,name)
...
for iTrk=1:length(tracks)
    ...
    int=zeros(size(vox,1),1);
    [~, ind] = unique(vox, 'rows');
    duplicate_ind = setdiff(1:size(vox, 1), ind);
    vox(duplicate_ind,:)=[];
    % Index into volume to extract scalar values
    inds                = sub2ind(header.dim, vox(:,1), vox(:,2), vox(:,3));
    scalars             = volume(inds);
    m(iTrk)=sum(scalars)./length(scalars);
    %int(ind)=scalars;
    %tracks(iTrk).matrix = [tracks(iTrk).matrix, int ];
end
