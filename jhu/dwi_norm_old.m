function dwi_norm(dwi, lesion, isTrace)
...
if ~exist('lesion','var')
 %lesion = spm_select(1,'^.*\.(gz|nii|voi)$','Select lesion image');
 lesion = spm_select(1,'^.*\.(voi)$','Select lesion image');
...
%save DWI data
if isempty(trace), nii_mat2ortho(matName); return; end;
