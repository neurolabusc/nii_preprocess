...
for s = 1: nSubjDir %1:nSubjDir2 %(nSubjDir2+1):nSubjDir    
    ...
    if ~isempty(imgs.T1) %&& ~isempty(imgs.Lesion) % ~isempty(imgs.DTI)
        ...
        %printSub(imgs,deblank(subjDirs{s}), lx == 1 );
...
nii_nii2mat(cstat, 12, matName);
nii_nii2mat(bstat, 13, matName);
%end doI3MSub()
...
nii_nii2mat(wFA, 6, matName);
nii_nii2mat(wMD, 8, matName);
%end doFaMdSub()
...
for tagOrder = 0:1  %1:-1:0
    [cbf, c1L, c1R, c2L, c2R] = nii_pasl12(asl, imgs.T1, tagOrder);
...
if exist(b,'file')
    fprintf('Skipping T1 file exists: %s\n', b);  %this stage was already run
else
