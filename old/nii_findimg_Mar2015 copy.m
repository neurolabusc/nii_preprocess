...
% function bedThreadsSub
% %https://www.jiscmail.ac.uk/cgi-bin/webadmin?A2=ind1503&L=FSL&P=R10541&1=FSL&9=A&J=on&d=No+Match%3BMatch%3BMatches&z=4
% %https://www.jiscmail.ac.uk/cgi-bin/webadmin?A3=ind0708&L=FSL&E=quoted-printable&P=2425429&B=--Apple-Mail-50--363685120&T=text%2Fhtml;%20charset=WINDOWS-1252&pending=
% maxThreads = feature('numCores');
% if maxThreads > 15, maxThreads = maxThreads - 1; end;
% fprintf('bedpost will use %d threads\n', maxThreads);
% str = '127.0.0.1';
% for i = 2: maxThreads, str = [str ' 127.0.0.1']; end;
% setenv('FSLMACHINELIST', str);
% %getenv('FSLMACHINELIST');
% setenv('FSLREMOTECALL','rsh');
% %end bedThreadsSub()
...
if false %ismac 
%     for i = 1: nROI
%         if mod(i,maxThreads) == 0, fprintf('probtrackx2 %d/%d (%d threads)\n', i, nROI, maxThreads); end;
%         if nOK(i) > 0 
%             maski=fullfile(mask_dir, [num2str(i),'.nii.gz']);
...
%     threadSub(threads, 0);
    ...
    for i = 1: nROI
        if nOK(i) > 0 
            ...
            command=sprintf('probtrackx2 -x "%s" --dir="%s" --forcedir  -P %d -s "%s" -m "%s" --opd --pd -l -c 0.2 --distthresh=0', ...
                maski, prob_diri, nPerm ,bed_merged, bed_mask );
            %command=sprintf('%sbin/probtrackx2 -x "%s" --dir="%s" --forcedir  -P %d -s "%s" -m "%s" --opd --pd -l -c 0.2 --distthresh=0', ...
            %    fslDirSub, maski, prob_diri, nPerm ,bed_merged, bed_mask );
            commands = [commands {command}];
            exitfiles = [exitfiles {fullfile(prob_diri, 'waytotal')}];
        end
    end 
    if false %ismac
        doThreadsMac(commands, exitfiles, maxThreads)

%         for i = 1: nROI
%             if mod(i,maxThreads) == 0, fprintf('probtrackx2 %d/%d (%d threads)\n', i, nROI, maxThreads); end;
%             if nOK(i) > 0 
%                 prob_diri=fullfile(prob_dir, num2str(i));
%                 status = doFslCmd ([commands{i} ], false);
%                 %xxx
%                 status
%                 waytotal =fullfile(prob_diri, 'waytotal'); %file created when thread completes
%                 threads = [threads; {waytotal}]; %#ok<AGROW>
...
%         threadSub(threads, 0);
        ...
        doThreadsSGE(commands, prob_dir);
%         command_file=fullfile(prob_dir, 'commands.txt');
%         log_file=fullfile(prob_dir, 'log.txt');
%         if exist(command_file, 'file'), delete(command_file); end;
%         fid = fopen(command_file, 'wt');
%         for i = 1 : numel(commands)
%             fprintf(fid,'%s\n',commands{i});
%         end
%         fclose(fid);
%         command=sprintf('fsl_sub -l %s -N probtrackx -T 1 -t %s', log_file, command_file );
%         doFslCmd (command, false);  
    end
end
...
function doThreadsSGE(commands, out_dir)
command_file=fullfile(out_dir, 'commands.txt');
log_file=fullfile(out_dir, 'log.txt');
if exist(command_file, 'file'), delete(command_file); end;
fid = fopen(command_file, 'wt');
for i = 1 : numel(commands)
    fprintf(fid,'%s\n',commands{i});
...
function doThreadsMac(commands, exitfiles, maxThreads)
if numel(commands) ~= numel(exitfiles) || numel(commands) < 1, return; end;
threads = {};
for i = 1: numel(commands)
	if mod(i,maxThreads) == 0, fprintf('thread %d/%d (%d threads)\n', i, numel(commands), maxThreads); end;
    doFslCmd ([commands{i} ' &'], false);
    threads = [threads; {exitfiles{i}}]; %#ok<AGROW>
    threads = threadSub(threads, maxThreads);
end
threadSub(threads, 0);
%doThreads()
