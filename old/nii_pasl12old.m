function [cbfName, c1L, c1R, c2L, c2R] = nii_pasl12 (ASLnames, T1name, reverseLabelTagOrder)
...
if ~exist('reverseLabelTagOrder','var'), reverseLabelTagOrder = false; end;
...
[T1nameBet, T1name] = priorSegnormSub(T1name); %bT1.nii, eT1_seg8.mat 
...
for ses = 1 : nses
    ...
    if ~isPASL %pCASL
        ...
        %FirstimageType = 0; %the first PASL volume after the M0 is labelled
        %FirstimageType =1;
        Delaytime = 1.2;
...
defname = fullfile(pth,['y_e' nam ext]); %enantimorphic normalization
if ~exist(defname,'file')
	defname = fullfile(pth,['y_' nam ext]); %standard normalization
	if ~exist(defname,'file')
...
%T1nameMat = fullfile(pth,[ nam, '_seg8.mat']);
