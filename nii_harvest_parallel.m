...
parfor i = 1:numLoops
   if(numLoops <= subjDirsSize)
       ...
       startParallelHarvest(subjDirs(start:fin),baseDir,outDir,setOrigin,isExitAfterTable,isPreprocess,isReportDims,reprocessfMRI,reprocessRest,reprocessASL,reprocessDTI,reprocessVBM); 
   else
       fprintf("Can't have more parallel loops than subjects to process...");
   end
 end

function      startParallelHarvest(subjDirs,baseDir,outDir,setOrigin,isExitAfterTable,isPreprocess,isReportDims,reprocessfMRI,reprocessRest,reprocessASL,reprocessDTI,reprocessVBM)
