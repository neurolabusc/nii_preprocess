...
for s = 1:size(subjDirs,1)% :-1: 1 %for each participant
    ...
    if exist(probDir,'file') && exist(baseDir, 'file')
       matName = [subj '.mat'];
       if  true %~isFieldSub(matName, 'dti_jhu')
...
for i = 1:(knROI-1)
    ...
    if ~isempty(hdri)
        ...
        if ~isempty(imgip)
            ...
            for j = i+1 : knROI
                [hdrj,imgj, voxj] = imgSub(maskDir, j);
                if ~isempty(hdrj)
                    [hdrjp,imgjp] = imgSubP(probDir, j);
                    if ~isempty(imgjp)
                        %mn_ij = mean(imgj (imgip > 0))
                        %mn_ij = mean(imgjp (imgi > 0))
                        OK = true;
...
function [hdri,imgi,voxi] = imgSubP(dir, index)
...
function [hdri,imgi,voxi] = imgSub(dir, index)
...
function mn = fslstatsKSub (img, mask)
%fslstats img -k mask
i = img(mask > 0);
i = i(i > 0); %non-zero mean
mn = mean(i);
%end fslstatsKSub()
