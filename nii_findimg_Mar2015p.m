function nii_findimg_Mar2015p (baseDir)
...
for s = 1: nSubjDir%1:nSubjDir2 %(nSubjDir2+1):nSubjDir    
    ...
    imgs.Rest = imgfindSub(imgs.Rest,'rest_', subjDir);
    ...
    if ~isempty(imgs.T1) && ~isempty(imgs.Lesion) % ~isempty(imgs.DTI)
       %printSub(imgs,deblank(subjDirs{s}),true ); continue; 

        t_start=tic; 
        doT1Sub(imgs, matName); %normalize T1
        doI3MSub(imgs, matName);
        t_start = reportTimeSub(t_start, 'T1');
        doAslSub(imgs, matName);
        t_start = reportTimeSub(t_start, 'ASL');
        doRestSub(imgs, matName, 1.85, 2); %TR= 1.850 sec, descending; %doRestSub(imgs, matName, 2.05, 5); %Souvik study 
        t_start = reportTimeSub(t_start, 'Rest');
        %doCp(imgs)
         dofMRISub(imgs, matName);
         t_start = reportTimeSub(t_start, 'fMRI');
         doDtiSub(imgs, matName);
         t_start = reportTimeSub(t_start, 'DTI');
        nii_fiber_quantify(matName, subjDir);
        t_start = reportTimeSub(t_start, 'FiberQuantify');
        doFaMdSub(imgs, matName);      
        t_start = reportTimeSub(t_start, 'FA');
    end
    %checkDims(imgs);
end
%end nii_findimg()

function t_start = reportTimeSub(t_start, cmd)
logname = 'elapsed_time.txt';
%logexists = exist(logname, 'file');
fid = fopen(logname,'a');
fprintf (fid, '%s took %f seconds to run.\n', cmd, toc(t_start) );
fclose(fid);
t_start=tic;
%end reportTimeSub()
...
function doCp(imgs)
baseDir = '/home/crlab/raw';
newDir = fullfile(baseDir, imgs.name);
if ~exist(newDir, 'file'), mkdir(newDir); end;
fields = fieldnames(imgs);
for i = 1:numel(fields)

    oldImg = imgs.(fields{i});
    if exist(oldImg,'file') == 2
        [p,n,x] = fileparts(oldImg);
        newImg = fullfile(newDir, [n, x]);
        copyfile(oldImg, newImg);
        x = '.bvec';
        oldImg = fullfile(p,[n,x]);
        newImg = fullfile(newDir, [n, x]);
        if exist(oldImg,'file'), copyfile(oldImg, newImg); end;
